console.log("adesh")

const URL="http://localhost:8080/"
let accountId="acc123";
var accountName="adesh sengar";
accountCity="delhi";

// new method to log the values
console.table([URL,accountId,accountName,accountCity]);

// URL=23;  // not allowed

let name;
console.log(name);

// primitive data type.
// number , string ,boolena, bigInt, null= > standard value, undefined, symbol

// object
console.log(typeof undefined);  //undefined is of undefined type
console.log(typeof null);  // null is of object type


// null coelsing operator.
var1=null??80;
var2=undefined ?? 10 ?? "database connecetion logic";
console.log(var2);
console.log(var1);
